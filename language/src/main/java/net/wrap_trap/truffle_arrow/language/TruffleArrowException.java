...
public class TruffleArrowException extends AbstractTruffleException {

  private static final long serialVersionUID = -6799734410727348507L;
  private static final InteropLibrary UNCACHED_LIB = InteropLibrary.getFactory().getUncached();

  @CompilerDirectives.TruffleBoundary
  public TruffleArrowException(String message, Node location) {
   ...
   */
  @CompilerDirectives.TruffleBoundary
  public static TruffleArrowException typeError(Node operation, Object... values) {
    ...
    if (operation != null) {
      NodeInfo nodeInfo = TruffleArrowLanguage.lookupNodeInfo(operation.getClass());
      if (nodeInfo != null) {
    ...
    for (int i = 0; i < values.length; i++) {
       ...
       */
      Object value = TruffleArrowLanguageView.forValue(values[i]);
      result.append(sep);
      sep = ", ";
      if (value == null) {
        result.append("ANY");
    ...
    return new TruffleArrowException(result.toString(), operation);
  }
}
